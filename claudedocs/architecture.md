...
1. ユーザーがゲーム作成フォームを入力
2. クライアント側でスコア計算
3. Firestore にゲームドキュメント作成
   - games コレクションに追加
   - userId に現在のユーザーIDを設定
4. 10個のフレームドキュメントを作成
   - games/{gameId}/frames サブコレクションに追加
5. リアルタイムでUIに反映
```
...
2. Firebase Authentication がGoogleログインポップアップを表示
3. ユーザーがGoogleアカウントで認証
4. Firebase Authentication がIDトークンを発行
5. アプリがIDトークンを検証
6. Firestore の users コレクションに情報を保存
7. ログイン状態をZustandに保存
```
...
### データ保護
- HTTPS通信必須（Firebase Hostingが自動対応）
- XSS対策（Reactが自動エスケープ）
- CSRF対策（Firebase Authenticationが自動対応）
- NoSQL インジェクション対策（Firebase SDKがパラメータバインディング）

## 拡張性設計

### 将来的な機能追加への対応

#### Phase 2: グループ・ランキング機能追加
```javascript
// groupsコレクションを追加
// membersサブコレクションで多対多関係を実現
```

#### Phase 3: 統計・分析機能
- Cloud Functions でデータ集計
- Firestore クエリで統計情報取得
- グラフ生成機能の実装

#### Phase 4: 複数認証プロバイダー対応
- Firebase Authenticationが自動管理
- Apple ID、Twitter、Microsoft認証の追加
- 1ユーザーが複数の認証方法を紐付け可能

### スケーラビリティ
- Firestoreの自動スケーリング
- Firebase Hostingの自動CDN配信
...
- Cloud Logging（Cloud Functionsのログ）
- カスタムログ（Reactアプリ内）
...
# 3. デプロイ確認
# Firebase Console でホスティングURLを確認
```
...
### Firebase vs FastAPI + PostgreSQL

| 項目 | Firebase | FastAPI + PostgreSQL |
|------|----------|---------------------|
| バックエンド開発 | 不要 | 必要 |
| データベース設計 | Firestore (NoSQL) | PostgreSQL (RDB) |
| 認証実装 | Firebase Auth | 自前実装 |
| インフラ管理 | 不要 | 必要 |
| スケーリング | 自動 | 手動 |
| リアルタイム | 標準 | 追加実装必要 |
| コスト（小規模） | 無料 | サーバー費用 |
